      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center text-slate-900 dark:text-white font-bold text-lg ${getScoreBg(audit.overallScore)}`}
            >
              {audit.overallScore}
            </div>
          ...
          <div className="flex justify-between items-center">
            {metrics.slice(0, 4).map((metric) => (
              <div key={metric.key} className="text-center">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-slate-900 dark:text-white text-xs font-bold ${getScoreBg(metric.score)}`}
                >
                  {metric.score}
                </div>
